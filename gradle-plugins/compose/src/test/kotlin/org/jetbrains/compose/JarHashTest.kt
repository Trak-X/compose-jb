...
import org.jetbrains.compose.desktop.application.internal.files.jarHash
...
class JarHashTest {
    ...
    @Test
    fun testJarHashIsNotAffectedByOrder() {
        val input1 = inputFile("input1.txt", "1")
        val input2 = inputFile("input2.txt", "2")

        val initJar =  createJar("init", input1, input2)
        val modifiedJar = createJar("modified", input2, input1)

        val initHash = jarHash(initJar)
        val modifiedHash = jarHash(modifiedJar)
        Assert.assertEquals(modifiedHash, initHash)
    }

    @Test
    fun testJarHashIsNotAffectedByTimeStamps() {
        val input1 = inputFile("input1.txt", "1")

        val initJar =  createJar("init", input1)
        check(input1.lastModified() != 0L)
        input1.setLastModified(0)
    ...
    @Test
    fun testJarHashIsAffectedByContent() {
        val input1 = inputFile("input1.txt", "1")
